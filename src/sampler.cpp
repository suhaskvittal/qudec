...
using DemType = stim::DetectorErrorModel;
...
Problem
generate_syndromes_with_k_errors(const DemType& dem, size_t k, size_t count, Rng& rng)
{
    ...
    using PqType = std::priority_queue<pq_entry, std::vector<pq_entry>, pq_cmp>;

    // Initialize the priority queue:

    PqType sampled_errors;
    ...
    SyndromeTable dets(count, dem.count_detectors());
    SyndromeTable obs(count, dem.count_observables());
    size_t error_idx{0};
...
Poly
compute_probability_polynomial(const DemType& dem, size_t max_errors)
{
