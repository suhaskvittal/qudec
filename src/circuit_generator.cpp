...
stim::Circuit
si1000(uint32_t distance, uint32_t rounds, double p, bool is_memory_x)
{
    using namespace stim;

    if (rounds < 1)
        throw std::invalid_argument("si1000: need rounds >= 1.");
    if (distance < 2)
        throw std::invalid_argument("si1000: need distance >= 2.");
    if (p < 0 || p > 1)
        throw std::invalid_argument("si1000: need 0 <= p <= 1.");
